...
cart_router.post('/payment/add', async (req, res) => {
    ...
    var payment_adding = payment_obj.payement_add(card_number,name_on_card,cvv,expiration,user_hash);
...
cart_router.post('/payment/checker', async (req, res) => {
    var user_hash = req.body.User_hash;
    var payment_checker = payment_obj.payement_checker(user_hash);
    if (payment_checker['valid']){
        return res.status(200).json({'message':payment_checker['message']});
    }
